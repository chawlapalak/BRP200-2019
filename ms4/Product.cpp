...
namespace ama {
	...
	bool ama::Product::operator>(const iProduct & other) const
	{
		if(strcmp(prName, other.name()) > 0)
            return true;
        return false;
	}
	...
	std::istream & ama::Product::read(std::istream & in, bool interractive)
	{
		...
		if (!interractive) {
			...
			if (!in.fail())
			{
				Product temp = Product(sku, name, unit,priceBeforeTax , qtNeeded, qtOnHand, taxable);
				*this = temp;
			}
		}
